...
const LABEL_TRANSLATIONS = {
    "factions-and-careers": "Factions and Careers",
    "ventures": "Ventures",
    "factions": "Factions",
    "items": "Items",
    "species": "Species",
    "profession-features": "Profession Features",
    "professions": "Professions",
    "species-features": "Species Features",
    "spells": "Spells",
    "historia-factions": "Factions (Historia)",
    "items-historia": "Items (Historia)",
    "spells-historia": "Spells (Historia)"
};

const COMPENDIUM_MAP = {
    "world.historia-factions": "historia-en.factions",
    "world.historia-items": "historia-en.items",
    "world.historia-spells": "historia-en.spells",
    "world.historia-ventures": "historia-en.ventures",
    "world.historia-factions-and-careers": "historia-en.factions-and-careers",
    "world.historia-species": "historia-en.species",
    "world.historia-professions": "historia-en.professions",
    "world.historia-profession-features": "historia-en.profession-features",
    "world.historia-species-features": "historia-en.species-features"
};
...
async function main() {
    ...
    const buildDir = "historia-en";
    await fs.mkdir(`${buildDir}/packs`, { recursive: true });
    let packs = [];
    ...
    const resolveImagePath = async (val) => {
        ...
        const potentialMedia = [
            ...
            decodedVal.replace(/^\/?modules\/historia-en\//, ""),
    ...
    for (const packDirName of packDirs) {
        ...
        // Skip Italian packs
        if (packDirName.endsWith("-it")) continue;

        // Handle names like "spells-historia" -> "spells"
        const base_name = packDirName.replace("-historia", "").replace("historia-", "");
        const pack_name = base_name;
        let pack_label = LABEL_TRANSLATIONS[packDirName] || base_name.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
        if (!pack_label.includes(labelSuffix)) pack_label += labelSuffix;

        console.log(`Processing pack directory ${packDirName} as ${pack_name}...`);
        ...
        const tempSrcDir = path.join("temp_packs_en", pack_name);
        const outDir = path.join(buildDir, "packs", pack_name);
    ...
    try {
        await copyFileAndDirs("src/scripts/main.js", `${buildDir}/scripts/main.js`);
    ...
    for (const chunk of chunks) {
        await Promise.all(chunk.map(async ([srcPath, moduleRelPath]) => {
            const destPath = path.join(buildDir, moduleRelPath);
            try {
    ...
    console.log(`English module generation complete! Copied ${copiedCount} images. (${missingCount} not found locally)`);
    await fs.rm("temp_packs_en", { recursive: true, force: true });
}
